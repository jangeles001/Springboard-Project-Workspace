...
function App ()
{
	...
	//Create new array based on current filterView state.
	//filteredMissions is used to create the components.
	const filteredMissions = missions.filter((mission) => 
    filterView === "All" ? true : mission.status === filterView
   	);

 	function updateMissionStatus(id, newStatus) {
    		setMissions((prevMissions) =>  //take previous state of missions.
      			prevMissions.map((mission) =>	//loop through to find matching id.
        		mission.id === id ? { ...mission, status: newStatus } : mission //set mission status to newStatus.
      		)
    	);
  	}
	
	return (
		<MissionControl title="Space Mission Control" missions={filteredMissions} filters={FILTERS} setFilterView={setFilterView} updateMissionStatus={updateMissionStatus}/>
	);
}
